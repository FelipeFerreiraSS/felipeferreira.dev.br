...
import { deletePost, fetchUserPostsList } from "@/store/features/post/truckFunctions";
...
export default function Posts() {
  ...
  const sortedData = [...(postsState?.userPosts || [])].sort((a, b) => {
    if (sortConfig.key) {
  ...
  const allTags = Array.from(
    new Set(postsState?.userPosts?.flatMap((post) => post.tags.map((tag) => tag.name)))
  );
  ...
  useEffect(() => {
    if (postsState.userPosts !== null && postsState.userPosts !== undefined) {
      setLoading(false);
    }
  }, [postsState.userPosts]);

  useEffect(() => {
    if (!postsState.userPosts) {
      dispatch(fetchUserPostsList());  
    }
  }, [])
  ...
  if (loading || !postsState.userPosts) {
    return (
    ...
    <Layout pageTitle="Posts">
      <div>
        <div className="flex flex-col sm:flex-row justify-between mb-5">
          ...
          >
            <Link href={'/dashboard/editor/post/create-post'} className="flex items-center justify-center">
            ...
            <TableBody>
              {paginatedData?.map((post) => (
                <TableRow key={post.id}>
                  ...
                  <TableCell>
                    <Link href={`/dashboard/editor/post/edit-post/${post.id}`}>
